...
const submitForm = (formEl) => {
  ...
  formEl.validate(async(valid) => {
    if (valid) {
      ...
      let  {data}= await request.post("user/regist",ruleForm2)
      if(data.code == 200){
            // 注册成功跳转 登录页
      ...
      <div class="Star">
        ...
        <div class="btu3">
          <router-link to="/regist" style="margin-right: 10px;">
            <el-button class="btu4" type="primary" round>Job Seeker</el-button>
          </router-link>
...
<style scoped>
      ...
      .left {
        position: fixed; /* 使用固定定位，让图片始终保持在浏览器窗口右侧 */
        top: 430px; /* 设置图片容器顶部距离浏览器窗口顶部的距离 */
        left: 120px; /* 设置图片容器右侧距离浏览器窗口右侧的距离 */
        transform: translateY(-50%); /* 使用 translateY(-50%) 居中图片容器 */
        font-family: Lucida Sans Unicode; /* 使用 Indie Flower 艺术字体 */
        font-size: 24px; /* 设置字体大小 */
        color: #030314; /* 设置字体颜色 */
